...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (!usernameRegex.test(username)) {
    return json({ error: "Username hanya boleh menggunakan huruf kecil, angka, dan underscore." }, { status: 400 });
  }
  ...
  try {
    ...
    try { await ensureWallet(db, userId); } catch (e) { console.log('ensureWallet failed (ignored):', e); }

    log.info?.('register.success', { userId });

    return redirect("/masuk?success=Akun berhasil dibuat, silakan masuk");
  } catch (error) {
    log.error?.('register.error', { error: error instanceof Error ? error.message : String(error) });
    return json({ error: "Terjadi kesalahan saat membuat akun. Silakan coba lagi." }, { status: 500 });
  }
}
...
export default function DaftarPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
      ...
      >
        <Card> {/* Kelas 'mx-auto' dan 'max-w-sm' dipindahkan ke motion.div */}
          <CardHeader>
