-- Migration 0020: Fix / reinforce ulama seed & repair malformed JSON from 0018
-- Gunakan ini jika 0018 sudah ditandai applied tetapi data kosong / sebagian gagal.
-- Aman idempotent (INSERT OR IGNORE + UPDATE bersyarat).
-- Jalankan: wrangler d1 migrations apply inti-santri (tambahkan --remote untuk remote DB)

BEGIN TRANSACTION;

-- 1. Pastikan kategori minimal ada (ulang OR IGNORE)
...
-- 2. Seed / perbaikan data ulama (gunakan bentuk aman tanpa apostrof rusak pada JSON)
...
-- 3. Pastikan entri kunci lain juga ada (jika hilang karena rollback 0018)
...
VALUES
 ...
 ('u_ahmad','cat_hanbali','8gdEHkswuvfxngOlfXPzz','Imam Ahmad bin Hanbal','Ahmad bin Muhammad','imam-ahmad-bin-hanbal','164 H','241 H','Baghdad','Imam hadits dan penjaga sunnah saat mihnah.','Musnad besar & keteguhan aqidah.','Al-Ilmu laisa bisurati ar-riwayah, innamal ilmu khosyah.',json('["Musnad Ahmad","Siyar Alam An-Nubala"]'),'3H',lower('Imam Ahmad bin Hanbal Ahmad bin Muhammad Imam hadits dan penjaga sunnah saat mihnah. Musnad besar & keteguhan aqidah.'),strftime('%s','now')),
...
-- 5. Backfill search_index jika ada baris kosong
